 // common-ui.js
import { auth, db } from "./firebase-config.js"; import { signOut } from " https://www.gstatic.com/firebasejs/11.0.2/firebase-auth.js  "; import { doc, getDoc } from " https://www.gstatic.com/firebasejs/11.0.2/firebase-firestore.js  "; import { onAuthStateChanged } from " https://www.gstatic.com/firebasejs/11.0.2/firebase-auth.js  ";
// --- GLOBAL FUNCTIONS ---
// Fungsi navigasi halaman window.goPage = (page) => { window.location.href = page; };
// Fungsi logout terpusat window.logout = async () => { try { await signOut(auth); localStorage.removeItem("userRole"); // Hapus role saat logout localStorage.removeItem("loginTime"); // Hapus waktu login juga window.location.href = "index.html"; // Kembali ke halaman login } catch (error) { console.error("Error logging out:", error); alert("Gagal logout. Silakan coba lagi."); } finally { // Penting: Pastikan timer dihentikan saat logout (baik manual atau otomatis) clearTimeout(inactivityTimer); } };
// --- INACTIVITY AUTO-LOGOUT LOGIC --- const INACTIVITY_TIMEOUT_MS = 10 * 60 * 1000; // 10 menit dalam milidetik let inactivityTimer;
function startInactivityTimer() { clearTimeout(inactivityTimer); // Hapus timer yang ada sebelumnya inactivityTimer = setTimeout(() => { console.log("Deteksi inaktivitas, otomatis logout..."); window.logout(); // Panggil fungsi logout yang sudah ada }, INACTIVITY_TIMEOUT_MS); }
function resetInactivityTimer() { startInactivityTimer(); // Cukup mulai ulang timer setiap kali ada aktivitas }
// Tambahkan event listener global untuk mendeteksi aktivitas pengguna // Ini hanya perlu dilakukan sekali saat script dimuat document.addEventListener('mousemove', resetInactivityTimer); document.addEventListener('keypress', resetInactivityTimer); document.addEventListener('click', resetInactivityTimer); document.addEventListener('scroll', resetInactivityTimer); // Opsional, baik untuk halaman panjang
// --- RENDER HEADER (NAVIGASI) --- function renderHeader(userRole, currentPageTitle, userName = 'Pengguna') { const headerPlaceholder = document.getElementById('header-placeholder'); if (!headerPlaceholder) return;
let adminMenuLinks = '';
...
// --- RENDER FOOTER --- function renderFooter() { const footerPlaceholder = document.getElementById('footer-placeholder'); if (!footerPlaceholder) return;
const footerHTML = `
    <footer>
...
// --- RENDER HOME MENU ITEMS (khusus untuk home.html) --- function renderHomeMenuItems(userRole, mainMenuGridId) { const mainMenuGrid = document.getElementById(mainMenuGridId); if (!mainMenuGrid) return;
...
// --- MAIN INITIALIZER FUNCTION --- // Fungsi ini dipanggil dari setiap halaman HTML export async function initPage(pageTitle, mainContentId, requiredRole, homeMenuGridId = null) { const mainContentElement = document.getElementById(mainContentId); if (!mainContentElement) { console.error( Elemen dengan ID '${mainContentId}' tidak ditemukan. ); return; }
